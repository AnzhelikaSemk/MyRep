...
questions = [
    Question("Вопрос 1 :Имя тети Гарри Поттера", 'Петунья', 'Сьюзен', "Александра", "Виктория"),
    Question("Вопрос 2:Какого цвета волосы у Гарри Поттера", 'Черные', 'Рыжие', "Каштановые", "Лысый"),
    Question("Вопрос 3:Имя дяди Гарри Поттера", 'Вернон', 'Его нет', "Александр", "Симус"),
    Question("Вопрос 4:Имя жены Гарри Поттера", 'Джинни', 'Сьюзен', "Гермиона", "Ее нет")
...
lb_Correct = QLabel(questions[question_number].question) # здесь будет написан текст правильного ответа
...
def next_question():
    ...
    lb_Correct.setText(questions[question_number].right_answer)
def show_answer () :
    if btn_OK.text() == 'Ответить':
        ...
        if check_answer():
           ...
           lb_Result.setText('НЕ верно!')
        lb_Correct.setText(f"Верный ответ под номером {answers[0].text()}")
...
app.exec()pyinstaller --onefile my_memory_card.py